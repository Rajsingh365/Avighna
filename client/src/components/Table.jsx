...
function Table({className, head_list, data_list}) {
  return (
    <div className={`${className}`}>
      <table className="w-[30%] md:w-[60%] text-sm text-left text-gray-500 dark:text-gray-400">
        <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
          <tr>
            {head_list.map((name,index)=><th scope="col" key={index} className="px-6 py-3">{name}</th>)}
          </tr>
        </thead>
        <tbody>
